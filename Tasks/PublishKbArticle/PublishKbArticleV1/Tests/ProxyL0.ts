...
describe('servicenow-http: agent proxy support', function () {
    ...
    it('azure-pipelines-terraform#1114 residual: rejects an untrusted/self-signed server -- servicenow-http.ts sets no explicit rejectUnauthorized, so this proves the Node default actually verifies', async () => {
        const target = https.createServer({ cert: TLS_CERT, key: TLS_KEY }, (_req, res) => {
            res.statusCode = 200;
            res.end('{"result":{"ok":true}}');
        });
        ...
        try {
            ...
            // flipped), this would start resolving instead of rejecting. Same
            // test as sethbacon/azure-pipelines-terraform#1127 -- this copy of
            // servicenow-http.ts is the one the source is migrating to.
            await assert.rejects(
                snRequest('GET', `https://127.0.0.1:${targetPort}/api/now/table/kb_knowledge`),
