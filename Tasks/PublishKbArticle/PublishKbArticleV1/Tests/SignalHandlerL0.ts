...
describe('index.ts SIGTERM/SIGINT registration -- re-raise after (no-op) cleanup (azure-pipelines-terraform#1113, suite-scope residual)', function () {
  ...
  const origReaders = {
    ...
    getPathInput: tasks.getPathInput,
    getBoolInput: tasks.getBoolInput,
    getDelimitedInput: tasks.getDelimitedInput,
  };
  ...
  beforeEach(() => {
    ...
    // Every input reader, not just getInput: this repo's ChangelogV1 reads
    // getPathInput first, so an unstubbed reader would let run() continue into
    // real git/ADO I/O -- the exact background-leak hazard described above.
    for (const reader of ['getInput', 'getPathInput', 'getBoolInput', 'getDelimitedInput'] as const) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (tasks as any)[reader] = () => {
  ...
  afterEach(() => {
    ...
    p.kill = origKill;
    p.exit = origExit;
    for (const [name, fn] of Object.entries(origReaders)) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (tasks as any)[name] = fn;
    }
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
