 ...
 * (letters, digits, hyphens only -- see the `InvalidInstance` check where it is
 * read from the endpoint/task input) before ever reaching this function, so it
 * cannot carry a scheme, a path, another host, or a redirect target -- only a
